...
require FRPATH . 'layerDatabase' . DS .'objects'. DS .'sql_object.php';
 ...
 *        
 */
class sql_query_object extends sql_object
{
    ...
    public function where($table, $column, $conditions)
    {
        if (is_string($table) && is_string($column) && is_array($conditions)) {
            ...
            if (($wSql && isset($conditions[0]) && isset($conditions[1])) || (! $wSql && isset($conditions[0]) && isset($conditions[1]) && isset($conditions[2]))) {
                
                if ($wSql) {
                    
                    if (! preg_match("/^=$|^>$|^<$|^>=$|^<=$|^<>$|^!=$|^like$/", $conditions[0]) && ! (is_string($conditions[1]) || is_numeric($conditions[1]))) {
                        trigger_error("Malformed columns condition data", E_USER_ERROR);
                    }
                } else {
                    
                    if ((! $conditions[0] == "AND" || ! $conditions[0] == "OR") && ! preg_match("/^=$|^>$|^<$|^>=$|^<=$|^<>$|^!=$|^like$/", $conditions[1]) && ! (is_string($conditions[2]) || is_numeric($conditions[2]))) {
                        trigger_error("Malformed columns condition data", E_USER_ERROR);
                    }
                }
